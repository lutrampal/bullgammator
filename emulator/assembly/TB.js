  ...
  _transfer(nbGroup, trackGroup, nbTrack, nbBlock) {
    for (let i = 0; i < NB_OCTADS_PER_GROUP; ++i) {
      for (let j = 0; j < NB_MEMORY_PER_OCTAD; ++j) {
        this.bullGamma.groups[nbGroup].octads[i].getMemory(j).copyBlockValues(
          trackGroup.tracks[nbTrack].blocks[nbBlock].octads[i].getMemory(j), 0, NB_BLOCKS_PER_MEMORY, true)
      }
    }
  }
}
