...
export class WireEditorComponent implements OnInit {
  ...
  private linesNumber = 32; // NB_INST_SERIES_3 / 2
  private offsetX = 14;
  private offsetY = 20;
  ...
  validateSeries3Hex(): void {
    try {
      ...
      this.series3Emit.emit(this.edit.getConnectionsTable().getHexCode());
  ...
  onClick(xCoord: number, yCoord: number): void {
    ...
    for (let columnIndex = 0; columnIndex < 11; columnIndex++) {
      const loopColumn = this.columns[columnIndex];
      if (xCoord <= currentX) {
        return;
      }
      if (xCoord < currentX + loopColumn.width) {
        clickedColumn = columnIndex;
        break;
      }
      currentX += loopColumn.width;
    }
    ...
    const column = this.columns[clickedColumn];
    ...
    let side = -1;
    let centerX;
    const centerY = 15 + this.offsetY + clickedLine * this.rowHeight;
    ...
    if (column.label === 'Distrib') {
      const instructionIndex = clickedLine + (side === 1 ? this.linesNumber : 0);
    ...
    if (column.type === 0) {
      const instructionIndex = clickedLine + (column.offset > this.columns[5].offset ? this.linesNumber : 0);
      this.setHexValue(instructionIndex, column.hex, this.currentValue % 16);
      this.instEmit.emit(this.edit.getConnectionsTable().getInstructionCode(instructionIndex));
      this.draw();
      return;
    }
  }
  ...
  drawCornerConnections(context: any, corner: number[][], id: number): void {
    const distribColumn = this.columns[5];
    for (let hexValue = 1; hexValue < corner.length; hexValue++) {
      let centerX1 = this.offsetX + distribColumn.offset + (id < 2 ? distribColumn.circleOffsetX1 : distribColumn.circleOffsetX2);
      let centerY1 = 15 + this.offsetY + (hexValue + (id % 2) * 16) * this.rowHeight;
      const color = this.valuesColors[hexValue];

      for (const instHex of corner[hexValue]) {
        const instIndex = instHex[0];
        const hexIndex = instHex[1];
        const reverse = {
          0: 4,
          1: 3,
          2: 2,
        ...
        const columnIndex = reverse[hexIndex + 4 * Math.floor(instIndex / this.linesNumber)];
        const column = this.columns[columnIndex];
        const centerX2 = this.offsetX + column.offset + column.circleOffsetX1;
        const centerY2 = 15 + this.offsetY + (instIndex % this.linesNumber) * this.rowHeight;
  ...
  drawBackground(context: any): void {
    ...
    for (let columnIndex = 0; columnIndex < 11; columnIndex++) {
      ...
      if (column.label === 'Distrib') {
        for (let lineIndex = 0; lineIndex < this.linesNumber; lineIndex++) {
          ...
          const lineY = 15 + this.offsetY + lineIndex * this.rowHeight;
          const textY = 6 + lineY;
          const lineHex = Number(lineIndex % 16).toString(16).toUpperCase();
          context.fillText(lineHex, this.offsetX + column.offset + 5, textY);
          context.fillText(lineHex, this.offsetX + column.offset + 61, textY);

          const color = this.valuesColors[lineIndex % 16];
        ...
        for (let lineIndex = 0; lineIndex < this.linesNumber; lineIndex++) {
          ...
          const lineY = 15 + this.offsetY + lineIndex * this.rowHeight;
          const textY = 6 + lineY;
          const lineLabel = lineIndex + (columnIndex === 0 ? 0 : this.linesNumber);
          context.fillText(
            lineLabel,
    ...
    context.lineTo(
      this.offsetX + this.columns[10].offset + this.columns[10].width,
      this.offsetY + 2 + (this.linesNumber / 2) * this.rowHeight
    );
    context.stroke();
  ...
  fillCircle(context: any, x: number, y: number, color: string): void {
  ...
  dessConex(context: any, x1: number, y1: number, x2: number, y2: number, color: string): void {
    ...
    if (this.tensed) {   // connexion tendue = droite
      ...
      const x3 = (x1 + x2) / 2; // milieu geometrique
      let y3 = (y1 + y2) / 2;
      y3 += 0.55 * Math.abs(x1 - x3) ; // pragmatique, doit etre >0
      const a = x1 * (y2 - y3) - (x2 - x3) * y1 + x2 * y3 - x3 * y2;
      ...
      const x = -b / (2 * a);  // centre du cercle
      const y = -c / (2 * a);  // centre du cercle
      const r = Math.hypot(x - x1, y - y1); // rayon du cercle
      const angle1 = Math.atan2(y1 - y, x1 - x);
